...
impl Wallet {
    ...
    /// `last_account` returns the most recent `Account`.
    pub fn last_account(&self) -> Option<Account> {
        self.history.last().copied()
    }

    /// `update_history` updates the `Wallet`'s `Account` history.
    pub fn update_history(&mut self, value: u64) -> Result<()> {
        self.validate()?;

        let new_account = if let Some(account) = self.last_account() {
