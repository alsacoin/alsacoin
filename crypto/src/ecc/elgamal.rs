...
#[derive(Copy, Clone, Ord, PartialOrd, Debug, Default)]
...
impl PartialEq for CypherText {
    fn eq(&self, other: &CypherText) -> bool {
        self.to_bytes().ct_eq(&other.to_bytes()).unwrap_u8() == 1u8
    }
}

impl Eq for CypherText {}

impl PartialOrd for CypherText {
    fn partial_cmp(&self, other: &CypherText) -> Option<cmp::Ordering> {
        // NB: not constant-time
        Some(self.to_bytes().cmp(&other.to_bytes()))
    }
}

impl Ord for CypherText {
    fn cmp(&self, other: &CypherText) -> cmp::Ordering {
        // NB: not constant-time
        self.to_bytes().cmp(&other.to_bytes())
    }
}
